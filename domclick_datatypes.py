...
class SearchParameters(BaseModel):
    ...
    def _proceed(self, k, v) -> dict:
        ...
        if isinstance(v, tuple) and len(v) > 1 and (isinstance(v[0], int) or isinstance(v[1], int)):
            if k == 'price':
...
class DomclickAddress_v2(BaseModel):
    displayName: str
    subways: list[Any]
...
class DomclickDeveloper(BaseModel):
    name: str
    logo: Optional[str] = None
...
class DomclickOffer(Offer):
    ...
    developer: Optional[DomclickDeveloper] = None
    chat_available: bool
...
class DomclickGeneralInfo(BaseModel):
    area: float
    rooms: int
    maxFloor: int
    minFloor: int
...
class DomclickLocation(BaseModel):
    lat: float
    lon: float


class DomclickOffer_v2(Offer):
    id: int
    path: str
    offerId: int
    generalInfo: DomclickGeneralInfo
    address: DomclickAddress_v2
    location: DomclickLocation
    price: int
    offerType: str
    dealType: str
